...
import { getConnectionRequests, getNotifications, respondToConnectionRequest, deleteConnection } from "@/store/user/userAPI";
...
const NotificationWindow = () => {
  ...
  const handleConnectionResponse = async (connectionId: string, status: "accepted" | "rejected") => {
    try {
      if (status === "rejected") {
        ...
        await dispatch(respondToConnectionRequest({ connectionId, status })).unwrap();
      }
  ...
  const renderNotification = (notification: any, index: number) => {
    ...
    if (
      ...
      (connectionStatus !== "pending" || (connectionRequestId ? actionedConnections.includes(connectionRequestId) : false))
    ) {
      return null;
    }
      ...
      <Container key={index} className="px-[30px] py-4 border-b border-blue_e1e2ff flex gap-2 border">
        ...
        <Container className="flex-1">
          {type === "comment" && (
            <>
              <Text className="text-sm md:text-base font-semibold text-[#787774]">
                {senderName} <span className="font-normal">commented on</span> {bannerName}
              </Text>
              <span className="text-[10px] text-[#CBCACA]">{timestamp}</span>
              <Text className="text-sm text-[#6E6D69] mt-1">{comment?.content}</Text>
          ...
          {type === "interest" && (
            <>
              <Text className="text-sm md:text-base font-semibold text-[#787774]">
                {senderName} <span className="font-normal">showed interest in</span> {bannerName}
              </Text>
              <span className="text-[10px] text-[#CBCACA]">{timestamp}</span>
          ...
          {type === "reply" && (
            <>
              <Text className="text-sm md:text-base font-semibold text-[#787774]">
                {senderName} <span className="font-normal">replied in</span> {bannerName}
              </Text>
              <span className="text-[10px] text-[#CBCACA]">{timestamp}</span>
              <Text className="text-sm text-[#6E6D69] mt-1">{comment?.content}</Text>
            </>
          ...
          {type === "connection_request" && connectionRequestId && (
            <Container className="flex flex-col md:flex-row md:items-center md:justify-between">
              <Container>
                <Text className="text-sm md:text-base font-semibold text-[#787774]">
                  {senderName} <span className="font-normal">sent you a connection request</span>
                </Text>
                <span className="text-[10px] text-[#CBCACA]">{timestamp}</span>
              </Container>
              <Container className="flex gap-2 mt-2 md:mt-0">
                <Button
                  ...
                  onClick={() => handleConnectionResponse(connectionRequestId!, "rejected")}
                ...
                <Button
                  ...
                  onClick={() => handleConnectionResponse(connectionRequestId!, "accepted")}
                  disabled={actionedConnections.includes(connectionRequestId)}
                >
                  Accept
                </Button>
          ...
          {type === "connection_accepted" && (
            <>
              <Text className="text-sm md:text-base font-semibold text-[#787774]">
                {senderName} <span className="font-normal">accepted your connection request</span>
              </Text>
              <span className="text-[10px] text-[#CBCACA]">{timestamp}</span>
            </>
          )}
  ...
  return (
    <Container className="w-full h-full border rounded-[20px] border-blue_e1e2ff">
      <Container className="px-[30px] py-[18px] border-b border-blue_e1e2ff">
        <Text variant="h3" className="text-black text-base md:text-2xl font-semibold">
          Notifications
        </Text>
      </Container>
